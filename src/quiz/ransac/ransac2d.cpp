...
std::unordered_set<int> Ransac(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
	...
	for (int i = 0; i < maxIterations; i++){
		...
		if(index1==index2) ++index1 % cloud->points.size();
		double A = cloud->points[index1].y - cloud->points[index2].y;
		double B = cloud->points[index2].x - cloud->points[index1].x;
		double C = cloud->points[index1].x * cloud->points[index2].y - cloud->points[index2].x * cloud->points[index1].y;
		double dnom = std::sqrt((A*A+B*B));
		std::unordered_set<int> inliers;

		// Measure distance between every point and fitted line
		// If distance is smaller than threshold count it as inlier
		for(int j=0;j<cloud->points.size();j++){
			double d = std::fabs(A*cloud->points[j].x+B*cloud->points[j].y+C)/dnom;
